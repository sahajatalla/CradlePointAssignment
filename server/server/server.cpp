...
public:
	...
	static void find_primality_send_result(int id, unsigned num, SOCKET ClientSocket)
	{
		//bool result = do_is_prime(id, num);
		bool result = isPrime(num);
		std::string result_s;
		std::cout << "thread id is  : " << id << " socket is : " << ClientSocket << " : " << id << ", num is " << num << endl;
...
__declspec(dllexport) int start_server()
{
	server s;
	return s.start_server();
}

__declspec(dllexport)  bool isPrime(unsigned int n)
{
	return server::isPrime(n);
}
